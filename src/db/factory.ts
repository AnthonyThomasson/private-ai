...
// People factory
export const peopleFactory = factory
  .define(
    {
      props: {
  ...
  .props({
    name: ({ props }) =>
      faker.person.fullName({
        sex: props.gender === "male" ? "male" : "female",
      }) as string,
    description: ({ props }) =>
      faker.helpers.arrayElement(
        props.gender === "male" ? maleDescriptions : femaleDescriptions,
      ) as string,
  })
...
// Evidences factory
export const evidencesFactory = factory
  .define(
    {
      props: {
  ...
  .traits({
    ...
    timeOfDeath: {
      props: {
        ...
        description: () =>
          "The victim was killed at " + faker.date.recent().toLocaleString(),
      },
    },
  })
  .props({
...
export const murdersFactory = factory
  .define(
    {
      ...
      vars: {
        ...
        murderWeapon: async () =>
          await evidencesFactory.use((t) => t.murderWeapon).create(),
        timeOfDeath: async () =>
          await evidencesFactory.use((t) => t.timeOfDeath).create(),
        location: async () => await locationsFactory.create(),
      },
    },
  ...
  .after(async (murder, vars) => {
    await evidenceRelationshipsFactory
      .vars({
...
export const evidenceRelationshipsFactory = factory
  .define(
    {
      props: {
      ...
      vars: {
        ...
        reporter: async (): Promise<Person | null> =>
          await peopleFactory.create(),
      },
    },
    async (data) =>
      (await db.insert(evidenceRelationships).values(data).returning())[0],
  )
  .props({
    reporterId: async ({ vars }) => (await vars.reporter)?.id ?? null,
...
export const locationsFactory = factory.define(
  {
    props: {
      ...
      description: () =>
        "A bar named 'The Local'. It's a popular spot for locals and tourists alike. It's a bit run down, but has a cozy feel. It's a bit dark, but has a warm glow. It's a bit loud, but has a cozy feel. It's a bit dark, but has a warm glow.",
    },
    vars: {},
  },
...
export const personRelationshipsFactory = factory
  .define(
    {
      props: {
    ...
    async (data) =>
      (await db.insert(personRelationships).values(data).returning())[0],
  )
  .props({
