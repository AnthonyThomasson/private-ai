...
import { db } from ".";
import { evidencesFactory, locationsFactory, murdersFactory, peopleFactory, personRelationshipsFactory } from "./factory";
import { evidenceRelationships, evidences, locations, murders, people, Person, personRelationships } from "./schema";
...
const createRandomRelationship = async (person1: Person, person2: Person) => {
	const relationshipType = faker.helpers.arrayElement(['friend', 'family', 'colleague', 'acquaintance']);
...
export const seed = async () => {
  ...
  let suspects: Person[] = [];
  for (let i = 0; i < 5; i++) {
    const suspect = await peopleFactory.vars({
  ...
  // Murder location
  const murderLocation = locationsFactory.props({
    description: () => "The alley was narrow and grimy, squeezed between crumbling brick walls. A broken streetlamp cast a weak, flickering light over the cracked concrete, where scraps of paper and glass were scattered. The air hung heavy with the smell of oil and damp trash."
  }).create();
  await murdersFactory.props({
    description: () => "A figure lies sprawled across a cracked concrete floor, a gun tossed nearby."
  })
  .vars({
    ...
    murderWeapon: async () => await evidencesFactory.use((t) => t.murderWeapon).props({
      known: () => true,
      description: () => "A Glock 19 Gen5 featuring a matte black finish, a 4.02-inch barrel, and a 15-round magazine. It has standard Glock night sights, a smooth, no-finger-groove grip, and slight holster wear on the slide, showing it’s been carried but well-maintained. The trigger is crisp, and the internals are clean, making it ready for reliable everyday use."
    }).create(),
  }).create();
}